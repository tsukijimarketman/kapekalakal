  ...
  useEffect(() => {
    ...
    const fetchTx = async () => {
      try {
        ...
        // Filter only "cancelled" status transactions
        const cancelledTransactions = list.filter(
          (tx) => tx.status === "cancelled"
        );

        const sorted = [...cancelledTransactions].sort(
          (a, b) =>
                      ...
                      <div key={`${tx._id}-${index}`} className="flex gap-4">
                        ...
                        <div className="flex-shrink-0">
                          <img
                            ...
                            alt={item.name}
                            className="w-16 h-16 object-cover rounded-lg border border-[#e1d0a7] dark:border-[#7a4e2e] opacity-75"
                          />
                        </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex-1">
                      <p className="text-[#7a4e2e] dark:text-[#d0b274] text-sm">
                        ...
                        {tx.cancellationReason || "No reason provided"}
                      </p>
                      {tx.cancellationDate && (
                        <p className="text-[#996936] dark:text-[#d0b274] text-xs mt-1">
                          Cancelled on{" "}
                          {new Date(tx.cancellationDate).toLocaleDateString(
                            undefined,
                            {
                              year: "numeric",
                              month: "long",
