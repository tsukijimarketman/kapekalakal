...
const InTransit: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchTx = async () => {
      try {
        ...
        // Filter only "in_transit" status transactions
        const inTransitTransactions = list.filter(
          (tx) => tx.status === "in_transit"
        );

        const sorted = [...inTransitTransactions].sort(
          (a, b) =>
        ...
        setError(
          err?.response?.data?.message ||
            err?.message ||
            "Failed to load in-transit orders"
        );
      } finally {
  ...
  const orderReceived = async (id: string) => {
    try {
      setProcessingId(id);
      setError(null);
      await axios.put(
        `${import.meta.env.VITE_API_URL}/transactions/${id}/confirm-receipt`,
        {},
        { withCredentials: true }
      );

      // Optimistically remove from In Transit list (it will appear in Completed tab)
    ...
    return (
      <div className="p-8 text-center">
        ...
        <div className="text-[#7a4e2e] dark:text-[#d0b274] text-sm mt-2">
          Orders currently being delivered will appear here
        </div>
      </div>
    );
  }
                ...
                <div className="mt-4 pt-3 border-t border-[#e1d0a7] dark:border-[#7a4e2e]">
                  <div className="flex justify-between items-center">
                    <div className="flex-1">
                      ...
                      {tx.deliveryInfo?.trackingNumber && (
                        <p className="text-[#996936] dark:text-[#d0b274] text-xs mt-1">
                          Tracking: {tx.deliveryInfo.trackingNumber}
                        </p>
                      )}
                      {tx.deliveryInfo?.estimatedDelivery && (
                        <p className="text-[#996936] dark:text-[#d0b274] text-xs mt-1">
                          Est. delivery:{" "}
                          {new Date(
                            tx.deliveryInfo.estimatedDelivery
                    ...
                    <button
                      onClick={() => orderReceived(tx._id)}
                      disabled={processingId === tx._id}
                      className={`cursor-pointer px-4 py-2 text-white text-sm font-medium rounded-lg transition-colors duration-200 ${
                        processingId === tx._id
                          ? "bg-[#b28341]/60 cursor-not-allowed"
                          : "bg-[#b28341] hover:bg-[#996936]"
                      }`}
                    >
                      {processingId === tx._id
                        ? "Processing…"
